...
class EnvironmentCLI < Thor
	...
	def list
		env_list = Opticon::Schema::Environment.all
		if env_list.empty?
	...
	def describe
		env = Opticon::Schema::Environment.find_by_name(options[:org])
	...
	def create
		...
		if options[:production] and Opticon::Schema::Environment.find_by_production(true)
			puts "A production environment already exists"
			exit
		end

		# If no production environment exists then we must create that first
		unless options[:production] and not Opticon::Schema::Environment.find_by_production(true)
			puts "A production environment must be configured"
			exit
		end

		if Opticon::Schema::Environment.find_by_name(options[:name]) then
			puts "Salesforce organization #{options[:name]} already exists"
			exit
		end

		env = Opticon::Schema::Environment.create(options)
		env.init
